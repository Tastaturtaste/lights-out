...
impl LightsOutState {
    fn new(size: usize) -> Self {
        Self {
            size,
            activations: vec![false; size * size],
            lights: vec![false; size * size],
            switch_to_solve: vec![false; size * size],
        }
    }
...
fn main() -> Result<(), slint::PlatformError> {
    ...
    let lights_out_state = Rc::new(RefCell::new(LightsOutState::new(
        ui.get_button_field_size() as usize,
    )));
    ui.on_notify_tile_clicked({
        ...
        move |x, y| {
            ...
            let switch_individual_lights = ui.get_switch_individual_lights();
            if switch_individual_lights {
                lights_out_state.lights[idx] = !lights_out_state.lights[idx];
                for switch in &mut lights_out_state.switch_to_solve {
                    *switch = false;
                }
            } else {
                lights_out_state.activations[idx] = !lights_out_state.activations[idx];
                lights_out_state.lights =
                    light_buttons(lights_out_state.size, &lights_out_state.activations);
                lights_out_state.switch_to_solve[idx] = false;
            }
            ui.set_button_field_lights(
                std::rc::Rc::new(slint::VecModel::from(lights_out_state.lights.clone())).into(),
            );
    ...
    ui.on_notify_solve_clicked({
        ...
        move || {
            let ui = ui_handle.unwrap();
            let mut lights_out_state = state_ref.borrow_mut();
            let missing_lights = lights_out_state
                .lights
                .iter()
                .cloned()
                .map(|light_on| !light_on)
                .collect();
    ...
    ui.on_notify_switch_individual_lights_clicked({
        ...
        move |switch_individual_lights_on_click: bool| {
            ...
            // In this mode the activations will not stay in sync with the switched lights
            if switch_individual_lights_on_click {
                return;
            }
            // When switching back to the activation switching mode the activations have to be reassociated with the lights
...
fn light_buttons(size: usize, activations: &[bool]) -> Vec<bool> {
    ...
    let lights = (&lights_system_matrix)
        * (&SimpleBoolMatrix::new(activations.to_vec(), activations.len(), 1));
    lights.data
}
...
impl SimpleBoolMatrix {
    ...
    fn get_mut_rows(&mut self, selected_row_indices: &[usize]) -> Vec<&mut [bool]> {
        ...
        sorted_selected_row_indices_indices
            .into_iter()
            .map(|i| rows[i].take())
            .collect()
    }
    ...
    fn mul(self, rhs: Self) -> Self::Output {
        ...
        for i in 0..self.nrows {
            for j in 0..rhs.ncols {
                ...
                let switch = self_ith_row_iter
                    .zip(rhs_jth_col_iter)
                    .map(|(rowval, colval)| Switch(*rowval) * Switch(*colval))
                    .reduce(|acc, e| acc + e);
    ...
    for selected_idx in 0..A_rows.len() {
        ...
        for reduced_idx in (selected_idx + 1)..A_rows.len() {
            ...
            if reduced_row[selected_idx] {
                (*reduced_row).iter_mut().zip(selected_row.iter()).for_each(
                    |(reduced_val, &selected_val)| {
                        *reduced_val = (Switch(*reduced_val) - Switch(selected_val)).into()
                    },
                );
